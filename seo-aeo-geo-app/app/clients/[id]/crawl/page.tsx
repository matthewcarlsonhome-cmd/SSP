...
type WorkbenchPayload = {
  client: { id: string; name: string; website_url: string };
  workbench: {
    firecrawl: {
      crawl: { id: string; status: string; seed_url: string; credits_used: number | null; created_at?: string } | null;
      pageCount: number;
      schemaCount: number;
      schemaTypes: string[];
...
export default function ClientCrawlBrowserPage() {
  ...
  const pages = payload?.workbench.firecrawl.pages || [];
  ...
  async function loadWorkbench() {
    ...
    if (!response.ok) throw new Error(data.error || "Failed to load crawl data");
    setPayload(data);
    if (!selectedPageId && data.workbench?.firecrawl?.pages?.[0]?.id) {
      setSelectedPageId(data.workbench.firecrawl.pages[0].id);
    }
  }
  ...
  async function loadPage(pageId: string) {
    ...
    setError("");
    try {
      const response = await fetch(`/api/clients/${clientId}/site-crawl/pages/${pageId}`, { cache: "no-store" });
      const data = await response.json();
  ...
  useEffect(() => {
    loadWorkbench()
      .catch((err) => setError(err instanceof Error ? err.message : "Failed to load crawl data"))
  ...
  if (!payload) {
    return (
      <div className="mx-auto max-w-4xl py-20 text-center">
        <p className="text-sm text-muted-foreground">{error || "No client crawl data found."}</p>
  ...
  const crawl = payload.workbench.firecrawl.crawl;
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={loadWorkbench}>
              <RefreshCw className="h-4 w-4" />
              Refresh
            </Button>
            {crawl?.id && (
              <a href={`/api/clients/${clientId}/site-crawl/download?crawlId=${crawl.id}`}>
                <Button>
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        <Metric label="Pages" value={payload.workbench.firecrawl.pageCount} />
        <Metric label="Schema items" value={payload.workbench.firecrawl.schemaCount} />
        <Metric label="Credits" value={crawl?.credits_used ?? 0} />
        <Metric label="Status" value={crawl?.status || "not run"} />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Click a stored page to view markdown, clean HTML, raw HTML, schema, and metadata.
            </CardDescription>
          </CardHeader>
          <CardContent className="max-h-[720px] space-y-2 overflow-auto pr-1">
            {pages.length ? (
              ...
              <div className="rounded-lg border border-dashed p-6 text-sm text-muted-foreground">
                No stored pages yet. Go back to the client and run Firecrawl.
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-lg border bg-muted/20 p-3">
              ...
              <pre className="max-h-[640px] overflow-auto whitespace-pre-wrap break-words rounded-md bg-background p-4 text-xs leading-relaxed text-foreground">
                {artifactText || "No artifact content is available for this tab. If this was an older crawl, rerun Firecrawl from the client page to store the expanded artifacts."}
              </pre>
            </div>
          </CardContent>
        </Card>
